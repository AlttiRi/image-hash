...
{
    const iData = await getImageDataFromFS(resolve(`../img/kittens-3264x2448.jpg`));
...
{
    const iData = await getImageDataFromFS(resolve(`../img/bridge-500x320.jpg`));
...
{
    const iData = await getImageDataFromFS(resolve(`../img/black-bg-orthocanna-500x500.jpg`));
    {
        const hash = bHash(iData);
        t({
            result: hash.hex,
...
{
    const iData = await getImageDataFromFS(resolve(`../img/imagehash-1200x600.png`));
    {
        const hash = aHash(iData);
        t({
            result: hash.hex,
